...
def read_midi(filepath) -> List[int]:
    mido_obj = mid_parser.MidiFile(filepath)
    notes = mido_obj.instruments[0].notes

    # convert notes to pianoroll
    pianoroll = np.array(pr_parser.notes2pianoroll(notes, ticks_per_beat=mido_obj.ticks_per_beat), dtype=np.uint8)
    # convert array from [[0, 0, ... 0], [0, 0, ... 0]] to [[[0], [0], ... [0]], [[0], [0], ... [0]]]
    pianoroll = np.reshape(pianoroll, pianoroll.shape + (1,))

    # duplicate values of axis 2 from [0] to [0 0 0]
    pianoroll = np.repeat(pianoroll, 3, axis=2)

    return pianoroll

def write_midi(filepath, pianoroll) -> None:
    pianoroll = np.delete(pianoroll, np.s_[1:], 2)
    pianoroll = np.reshape(pianoroll, pianoroll.shape[:-1])
    notes = pr_parser.pianoroll2notes(pianoroll)
...
def initmorph(startimgpath,endimgpath,featuregridsize,subpixel,showfeatures,scale) :
    ...
    bar_length = 460
    leftMidi = read_midi(startimgpath)[-bar_length:]
    rightMidi = read_midi(endimgpath)[:bar_length]

    triangleTuple = loadTriangles(leftMidi, rightMidi, featuregridsize, showfeatures)

    # Morpher objects for color layers BGR
    morphers = [
                Morpher(leftMidi[:, :, 0], triangleTuple[0], rightMidi[:, :, 0], triangleTuple[1]),
                Morpher(leftMidi[:, :, 1], triangleTuple[0], rightMidi[:, :, 1], triangleTuple[1]),
                Morpher(leftMidi[:, :, 2], triangleTuple[0], rightMidi[:, :, 2], triangleTuple[1])
               ]
...
def morphprocess(mphs,framerate,outimgprefix,subpixel,smoothing) :
    ...
    for i in range(1,framerate) :
        ...
        # write file
        filename = outimgprefix+str(framecnt)+".mid"
        framecnt = framecnt + 1
        write_midi(filename, outimage)
...
mfeaturegridsize = 4 # number of image divisions on each axis, for example 5 creates 5x5 = 25 automatic feature points + 4 corners come automatically
mframerate = 5 # number of transition frames to render + 1 ; for example 30 renders transiton frames 1..29
...
margparser.add_argument("-inframes", default=minframes, help="REQUIRED input filenames in a list, for example: -inframes ['f0.png','f30.png','f60.png']")
margparser.add_argument("-outprefix", default=moutprefix, help="REQUIRED output filename prefix, -outprefix f  will write/overwrite f1.png f2.png ...")
